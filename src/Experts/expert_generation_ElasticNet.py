...
expert_EN_cv.fit(X_train,y_train)  # entrainement sur train
best_alpha = expert_EN_cv.named_steps["enet"].alpha_
best_l1 = expert_EN_cv.named_steps["enet"].l1_ratio_

print("Alpha optimal (lambda) :", best_alpha)
print("l1_ratio optimal (gamma) :", best_l1)

y_pred_1 = expert_EN_cv.predict(X_test) # prédiction
...
coeff_EN = expert_EN_cv.named_steps["enet"].coef_
intercept_EN = expert_EN_cv.named_steps["enet"].intercept_
...
coef = np.polyfit(y_test, y_pred_1, 1)
poly = np.poly1d(coef)
